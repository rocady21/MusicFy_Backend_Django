...
from .serializer import UserSerializer,FriendsSerializer,SerializerUserShow
from .models import Friends,Status
...
def login(request):
    ...
    if exist_user is not None:
        ...
        token, created = Token.objects.get_or_create(user=exist_user)
    ...
    # Establece la fecha de expiración del token
        ...
        if check_password(data["password"], user_date.data["password"]):
            data_user_send = SerializerUserShow(user_date.data)

            resp = requests.post('http://node_socekts-node_js-1:3000/emit_user_connected', json=data_user_send)
            return Response({
                "ok":True,
                "msg": "El user existe",
                "user": user_date.data,
